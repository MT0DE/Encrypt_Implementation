...
func encrypt(text []byte, key []byte) []byte{
	...
	if(len(text) % des.BlockSize != 0){
		text = padding(text, des.BlockSize)
	...
	iv := ciphertext[:des.BlockSize] //8 byte array
	...
	CBCmode.CryptBlocks(ciphertext[des.BlockSize:], plaintext)
...
func benchmark(files_to_encrypt [][]byte, key []byte){
	...
	fmt.Printf("Encrypting with 3DES %d times\n", int(iterations))
...
func test_aes_no_padding_but_correct_lenght(){
	filename := "secret.key"
	var key []byte = load_key(filename)
	key_n := make([]byte, 24)
	for i := 0; i < len(key_n); i++{
		if i < len(key){
			key_n[i] = key[i]
		} else{
			key_n[i] = byte(i)
	...
	fmt.Println(text)
	encrypt := encrypt(text, key_n)

	decrypted := decrypt(encrypt, key_n)
...
func main() {
	...
	benchmark(plaintext_files, key_n)
